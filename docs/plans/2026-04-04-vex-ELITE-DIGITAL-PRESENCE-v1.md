...
| Criterion                            | VLR note                                                                                                                                       |
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| **This file** §0–§31 + **VLR**       | Locked blueprint through **§31** (§30 Resource Arsenal + firepower; **§31** pitch deck mirror); component tree §23, perf §21, revenue §26–§27, Cox §28, autonomy §29 |
...
- **Single source of truth:** this file (**§0–§31**): `**VLR`**, **WebGL perf + 2026 luxury UX**, **revenue / investor**, **Cox §28**, **local autonomy §29**, **§30 Resource Arsenal** (live tooling + asset links) **+ firepower doctrine** (CI, Turbo, honest automation claims), **§31** (investor / marketing **pitch deck** — must stay aligned with **§26** / **§28.4**). Do not fork competing matrices in Slack or Notion without linking back.
...
- **Entry points** (canonical one-liners already wired): `PROJECT_SPACE.md`, `AGENTS.md`, `README.md`, `docs/SHIP.md`, `docs/ENGINEERING_REALITY.md` → **§29** (zero-click IDE autonomy), **§30** (Resource Arsenal + **Turbo remote cache** copy/paste setup + perf tools), **§28** (Cox), **§31** (pitch deck) as needed; plus [Crown Jewel v2.0](2026-04-05-vex-ELITE-DIGITAL-PRESENCE-v2.0.md), [v2 summary](2026-04-05-vex-ELITE-DIGITAL-PRESENCE-v2.md), [Apex Studio /build](2026-04-05-vex-apex-studio-configurator-v1.0.md).
...
- **Do not** duplicate §21–§31 into playbooks; **link** here for WebGL + luxury UX supremacy specs + **§30** velocity / tooling + **§31** deck narrative.

---

## 26. WebGL supremacy → revenue engine v3 (one-pager)

**One Solution → Revenue Engine v3:** one **tenant-safe** monorepo ships the **cinematic vault** (`vortex` + configure + `/build`) **and** the **GTM velocity stack** — **§29** (**Settings → Agent background = Always** + modal **don’t ask again** + **full Cursor quit/reopen** = **zero-click** autonomy), **§30** (**Turbo** cache warm-up + **seeded** remote cache when tokens/roles allow + `**turbo.json`** graph + **visual asset arsenal** + env `**NEXT_PUBLIC_HERO_VEHICLE_GLB`** → `**DynamicHeroShell` → vortex** flagship GLB without forking legacy video/CSS), plus **human + agent swarm** on this **§0–§31** spec. **External narrative mirror:** **§31** (“The Private Hypercar Vault” deck) uses the same **one-liner**, **visual supremacy**, **Revenue Engine v3**, and **Cox gap** beats — update **§31** when GTM positioning shifts so marketing and engineering stay aligned. The same **documentation-first** corpus **self-corrects** operational footguns (e.g. **Turbo `--dry` vs `--dry-run=text`**, **§29** trust note) so **onboarding + agents** spend less time on dead-end commands than teams reconciling **tribal wiki** across **multi-brand** stacks. That combination is the **honest** luxury-segment **production-rate** edge vs **acquisition-siloed** mass-market orgs (**§28**); it is **not** a claim to Cox-scale **DMS / auction** breadth until **named** integrations exist.
...
- **Revenue Engine v3 (internal frame):** **one unified luxury OS** (web + CRM + API) **plus** **production-rate firepower** — **zero-click local autonomy** (**§29**), **Turbo** warm-up + **seeded** remote cache + `**turbo.json`** (**§30**, `**TURBO_CACHE`** + **write-capable** tokens when seeding shared artifacts), **visual asset arsenal** + `**NEXT_PUBLIC_HERO_VEHICLE_GLB`** (vortex flagship without touching legacy `**HeroCinematicLayer**`), and **swarm parallelism** on the **§0–§31** blueprint — is the **repeatable** way VEX **out-ships** fragmented retail conglomerates **in-segment** without pretending to **out-platform** Cox on **DMS / wholesale** depth.
- **Positioning line (external):** *“VEX is the unified luxury automotive OS: one cinematic customer vault, one dealer cockpit, one tenant-safe API — built for vehicles and buyers that Cox’s mass-market stack was never designed to romance.”* **Deck subtitle (§31):** *“The single unified luxury automotive OS that turns browsers into buyers and dealers into evangelists.”* **Cover one-liner (§31):** *“One obsidian vault. One glass cockpit. One breathtaking digital-twin experience. The domain that draws global attention like no other.”*
- **Deck consistency:** Investor / marketing **slide beats** live in **§31** — when **Apex** pricing, **pilot** claims, or **Cox** framing change, edit **§28.4** + **§26** first, then **§31** so PDFs and the blueprint **do not drift**.
- **Speed advantage (luxury segment):** **One monorepo** + **single design language** + **cinematic-first** roadmap means VEX can **ship** obsidian-vault UX and **digital-twin** surfaces faster than **acquisition-siloed** incumbents can align a **coherent** HNW story — **not** faster at Cox-scale **DMS** depth (§28.1). **Production-rate firepower** compounds that: **§29** locks **zero-click** IDE velocity (**Agent: Run in Background → Always** + workspace **don’t ask again** + **full Cursor quit/reopen**); **§30** adds **Resource Arsenal** plus **Turbo remote cache** (`**TURBO_TOKEN` / `TURBO_TEAM`**, optional `**TURBO_CACHE=remote:rw**`, **write** role when seeding) on **local `.env.local` + GitHub Actions** and a **cache-aware `turbo.json`** (`build`, `**quality:web**`, `**cacheDir**`, safe `**globalPassThroughEnv**`) so **repeat builds / quality** amortize across **machines and CI** when auth and permissions allow — **~4–6s** is a **typical** cache-hit band, **not** a guarantee every run; `**NEXT_PUBLIC_HERO_VEHICLE_GLB`** + **visual asset arsenal** + **swarm parallelism** (people + agents) on **one** spec — still **honest** vs backend breadth gaps (verify with logs, not slogans).
...
- **Documentation as velocity:** A **living §0–§31** blueprint that **fixes** stale one-liners (CLI flags, cache auth, asset paths), states **non-negotiable** trust boundaries (**§29** **Security note**), and ships a **copy-paste** **§31** deck package is part of VEX’s **luxury-segment** speed — **less** rework from bad copy-paste than **siloed** product orgs where truth lives in **Slack** threads and **version-skewed** runbooks.
...
**Do not** run `**pnpm -w turbo run build --dry`** — Turbo **2.x** does **not** support `**--dry`** (command fails or is ignored). Copy-paste recipes that end in `--dry` are **wrong**; **§30** uses `**--dry-run=text`** only.
...
**Flagship vortex GLB (env, no binary in git):** set `**NEXT_PUBLIC_HERO_VEHICLE_GLB`** to a **HTTPS** URL or site-relative path such as `**/models/your-flagship.glb`** (file must live under `**apps/web/public/models/**`). `**resolveHeroVehicleGlbUrl()**` in `apps/web/src/lib/vehicle3d/defaults.ts` feeds `**VortexHeroScene**` on the path `**DynamicHeroShell` → `ApexHeroScene` / `VortexHeroScene**`. Stage source assets under `**packages/3d-configurator/assets/models/**`, then **copy** to `**public/models/`** or **upload** to CDN — Next does **not** serve the package `assets/` tree as HTTP.

**Copy-paste — drop a real PBR `.glb` + wire vortex (repo root):**

```bash
# 1) Put your file at: apps/web/public/models/your-flagship.glb
# 2) Point the vortex hero at it (append to .env.local — never commit):
echo 'NEXT_PUBLIC_HERO_VEHICLE_GLB=/models/your-flagship.glb' >> .env.local
# 3) Verify graph keys, then build + web smoke (correct Turbo 2 flags):
pnpm -w turbo run build --dry-run=text && pnpm -w turbo run build && pnpm exec turbo run quality:web --filter=@vex/web
```

Unset `**NEXT_PUBLIC_HERO_VEHICLE_GLB**` to restore the **Khronos ToyCar** default.

**Turbo cache warm-up (local or CI — populates `.turbo/cache` and remote when write-capable):**

**One-liner (build graph + web smoke, no GLB step):** `pnpm -w turbo run build && pnpm exec turbo run quality:web --filter=@vex/web`
...
1. **GitHub Actions:** **Settings → Secrets and variables → Actions** → add repository secrets `**TURBO_TOKEN`** and `**TURBO_TEAM**` (same values as Turbo dashboard). `**quality.yml**` passes these at **job** `env` so `**quality:web`** can **remote-cache** with the main CI graph.
2. **Verify:** run `**pnpm -w turbo run build`** twice; the second run should show **remote cache** usage and **must not** print `**Remote caching disabled`** (if it still does, secrets are missing or typo’d).
...
## 31. Pitch Deck Package — “The Private Hypercar Vault” (investor / GTM — copy-paste)

**Purpose:** Canonical **slide narrative** aligned with **§26**, **§28**, **§30**, and **VLR**. Paste into Keynote, PowerPoint, Google Slides, Pitch, or Figma; export PDF for data room or marketing agent. **Replace** `https://vex.example/…` with **live** URLs when deployed.

**Deck title:** VEX — Vortex Exotic Exchange  
**Subtitle:** The single unified luxury automotive OS that turns browsers into buyers and dealers into evangelists.  
**One-liner (cover):** One obsidian vault. One glass cockpit. One breathtaking digital-twin experience. The domain that draws global attention like no other.

### 31.1 Slide 1 — Cover

- **Headline:** VEX  
- **Subheadline:** The cinematic luxury automotive platform for high-net-worth collectors and premium dealers.  
- **Visual:** Obsidian void background with violet–gold sheen + subtle particle vortex formation (static export of the hero or a high-quality render).  
- **Tagline:** Where rarity meets velocity.

### 31.2 Slide 2 — The Problem

- Luxury buyers face fragmented, dated experiences.  
- Premium dealers struggle with slow, pushy, non-cinematic tools.  
- Cox Automotive and legacy DMS stacks optimize for mass-market throughput, not white-glove desire and emotional trust.  
- **Visual:** Split screen — gray cluttered legacy dealer UI vs clean obsidian VEX vault.

### 31.3 Slide 3 — The Solution

VEX is the single unified luxury OS:

- **Customer-facing** cinematic hypercar vault (`apps/web`)  
- **Private-jet** dealer CRM cockpit (`apps/crm`)  
- **Bulletproof** multi-tenant backend (`apps/api`)  
- **Shared** luxury design system + 3D configurator engine (`@vex/ui` + `@vex/3d-configurator`)

One product. One breathtaking DNA. From hero to checkout to deal desk — the same obsidian vault experience.

### 31.4 Slide 4 — Visual Supremacy (The Moat)

- Obsidian void + violet–gold foil energy  
- **60 fps** digital-twin vault hero (`DynamicHeroShell` → vortex path)  
- Real-time PBR configurator with flagship exotic models  
- Glassmorphic CRM orbs and metric cards  
- Liquid-metal CTAs and magnetic micro-interactions  

**Result:** Visitors feel they have stepped into a **$20M private showroom** — not another dealer website.

### 31.5 Slide 5 — Technical Firepower

- Monorepo with **Turbo remote cache** (seeded & live)  
- **Zero-click** local autonomy for the development swarm (**§29**)  
- Tenant-scoped **RBAC** + **Prisma** middleware  
- **BullMQ** async jobs for 3D asset provisioning and PDF generation  
- **Stripe** enterprise billing + valuation cost guardrails  

**Production rate:** From local build to live preview in minutes, not days.

### 31.6 Slide 6 — Revenue Engine v3

- **Apex tier** ($499/mo illustrative): white-label 3D portals and branded customer portal; custom domains + SSL; CRM premium lane with glass cinematic parity; higher valuation quota and priority support; **instant cinematic demo provisioning** on tenant creation (claim **live** only when `tenant-3d-demo-seed` + workers are **proven** per env — **§28.3**, **§30.3**).  
- **Pilot playbook:** 90-second self-serve onboarding with branded 3D demo assets pre-loaded (**target** until queue path is certified).  
- **Conversion hypothesis (to measure):** Cinematic vault + configurator depth drives significantly higher qualified traffic to checkout vs flat legacy experiences — **instrument**; do not claim a fixed **% lift** without powered experiments (**§26**).

### 31.7 Slide 7 — Competitive Positioning (Cox Gap)

We do not clone Cox’s mass-market empire. We own the luxury/exotic emotional + digital-twin layer Cox is not built to romance.

**VEX advantage:**

- Unified cinematic UX across web + CRM  
- Tenant-isolated 3D configurator + asset pipeline  
- Production-rate firepower (Turbo cache + swarm autonomy)  
- Honest differentiation on desire, rarity, and trust — not throughput breadth  

(Aligns with **§28** — never claim DMS/auction parity until named integrations ship.)

### 31.8 Slide 8 — Roadmap & Traction

**Immediate next:**

- Flagship exotic GLB in vortex hero (`NEXT_PUBLIC_HERO_VEHICLE_GLB` — **§30**)  
- Full Apex Studio configurator rollout  
- BullMQ tenant 3D demo seed job live  

90-second pilot onboarding live — ready for first luxury dealer groups (**when your VLR pillars are green on `main`**).

### 31.9 Slide 9 — Call to Action

**Join the private vault.**

- Pilot signup: https://vex.example/pilot  
- Investor deck / data room: https://vex.example/capital  
- Live site: https://vex.example  

**Contact:** [Your name] — Founder  
**Closing line:** The domain that draws global attention like no other.

### 31.10 How to use (marketing handoff)

1. Copy **§31.1–§31.9** into a new deck; add high-quality screenshots/renders (obsidian hero + configurator; vortex path when flagship GLB is set).  
2. Replace **vex.example** URLs with production routes.  
3. Send to your marketing agent with: *“Here is the complete pitch deck package for VEX (master plan **§31**). Use the one-liner narrative, visual supremacy slides, and revenue engine language exactly as written. Focus on the cinematic vault as the emotional differentiator. Let me know when the deck is polished and ready for distribution.”*