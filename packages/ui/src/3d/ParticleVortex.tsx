...
function fillVexClusters(target: Float32Array) {
  let i = 0;
...
function fillScatterSphere(scatter: Float32Array) {
  for (let i = 0; i < VEX_POINTS; i++) {
...
export function ParticleVortex({
  ...
  const { geometry, scatter, target } = useMemo(() => {
    ...
    fillVexClusters(targetArr);
    const scatterArr = new Float32Array(VEX_POINTS * 3);
    fillScatterSphere(scatterArr);
