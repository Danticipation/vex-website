...
referralRouter.post("/generate", requireAuth, validateBody(createReferralSchema), async (req, res) => {
...
referralRouter.post("/apply", requireAuth, async (req, res) => {
  if (!req.user) return res.status(401).json({ code: "UNAUTHORIZED", message: "Login required" });
  ...
  await prisma.referral.updateMany({
    where: { id: referral.id, tenantId: req.user.tenantId },
